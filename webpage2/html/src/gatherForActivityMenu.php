...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    while ($sql->fetch()) {
        ...
        if (!empty($suggestedFriendsArr)) {
            ...
            $hint .= "<div class=\"activity-card\"> <h3><a href=\"http://localhost/checkpoint/html/src/process_fetch_user.php?Username=%22{$randomFriend}%22\">{$randomFriend}</a> has mutual friends, do you know them?</h3></div>";
            $suggestedFriendsArr = [];
        }
        ...
        switch ($activityType) {
            case "review":
                ...
                break;
            case "friends":
                $message = ($relationshipType == "friends") ? " and you became friends on $date" : "requested to be your friend on $date";
                $form = ($relationshipType == "requested") ? "<form id=\"friendAction\" action=\"../src/processFriendRequests.php\" method=\"POST\"><button type=\"submit\" name=\"buttonContent\" value=\"accept\"> accept friend request? </button><input type=\"hidden\" name=\"reviewer\" value=\"$reviewer\"></form>" : "";
                $hint .= "<div class=\"activity-card\"> <h3><a href=\"http://localhost/checkpoint/html/src/process_fetch_user.php?Username=%22$reviewer%22\">$reviewer</a> $message</h3>$form</div>";
                break;
            case "comments":
                $hint .= "<div class=\"activity-card\" onclick=\"openReview($reviewID)\"> <h3><a href=\"http://localhost/checkpoint/html/src/process_fetch_user.php?Username=%22$reviewer%22\">$reviewer</a> commented on your post;</h3> <p>$review<br><br> commented on $date</p></div>";
                break;
            case "likes":
                if (!str_contains($reviewsLiked, "$reviewID,")) {
                    $likers = ($likes > 1) ? "$reviewer and " . ($likes - 1) . " others" : "$reviewer";
                    $hint .= "<div class=\"activity-card\" onclick=\"openReview($reviewID)\"> <h3><a href=\"http://localhost/checkpoint/html/src/process_fetch_user.php?Username=%22$reviewer%22\">$likers</a> liked your review of <a href=\"http://localhost/checkpoint/html/src/process_fetch_game.php?id=$index\">$title</a></h3></div>";
            ...
            case "likesComments":
                if (!str_contains($commentsLiked, "$reviewID,")) {
                    $likers = ($likes > 1) ? "$reviewer and " . ($likes - 1) . " others" : "$reviewer";
                    $hint .= "<div class=\"activity-card\" onclick=\"openReview($index)\"> <h3><a href=\"http://localhost/checkpoint/html/src/process_fetch_user.php?Username=%22$reviewer%22\">$likers</a> liked your comment;</h3><br><p>$review</p></div>";
